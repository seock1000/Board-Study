...
public class BoardArticleCountRepository {
    ...
    public void createOrUpdate(Long boardId, Long articleCount) {
        stringRedisTemplate.opsForValue().set(generateKey(boardId), String.valueOf(articleCount));
    }

    public Long read(Long boardId) {
        String result = stringRedisTemplate.opsForValue().get(generateKey(boardId));
